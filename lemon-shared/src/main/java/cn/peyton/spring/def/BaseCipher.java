...
import cn.peyton.spring.cipher.UnidirectionalCipherUtil;
...
public abstract class BaseCipher implements Serializable {
	...
	/** 使用hmac AES 内容加密key  模式 */
	public static final SymmetricCipherUtil.SymmetricCipher CIPHER_CONTENT_KEY_MODEL = SymmetricCipherUtil.SymmetricCipher.AES;
	...
	public static String encoderHMAC(String content) {
		return SymmetricCipherUtil.encoder(content, CIPHER_CONTENT_KEY, CIPHER_CONTENT_KEY_MODEL);
	}
	/**
	...
	public static String decoderHMAC(String content) {
		return SymmetricCipherUtil.decoder(content, CIPHER_CONTENT_KEY, CIPHER_CONTENT_KEY_MODEL);
	}
	/**
	 * <h4>单向加密{用在密码上}</h4>
	 * @param content 内容
	...
	public static String encoderMD5(String content,String key) {
		return UnidirectionalCipherUtil.MD5(content, key);
	}
